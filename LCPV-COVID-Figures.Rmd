...
Figure 1. Impact of COVID-19 on the Agricultural Systems in LCPV.
```{r Fig 1 - Ag system}
...
plot_grid(nrow = 2, rel_heights = c(1, .4),
  ...
  ggsave(plot = ., filename = "Fig_AgSystemsv2.pdf", units = "in", height = 10, width = 12, dpi = 600, device = "pdf") # pdf for publication
```

Figure 2. Impact of COVID-19 on agricultural production.
```{r Fig 2 - Production}
...
Figure 3. Impact on forest ecosystems.
```{r Fig 3 - Forest ecosystems}
